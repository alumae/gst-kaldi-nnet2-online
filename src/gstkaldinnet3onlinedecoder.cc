...
/**
 * SECTION:element-kaldinnet3onlinedecoder
 *
 * Converts speech to text using Kaldi's SingleUtteranceNnet3Decoder.
 *
 * <title>Example launch line</title>
 * |[
 * GST_PLUGIN_PATH=. gst-launch-1.0 --gst-debug="kaldinnet3onlinedecoder:5" -q \
 * filesrc location=123_456.wav ! decodebin ! audioconvert ! audioresample ! \
 * kaldinnet3onlinedecoder model=nnet3_online_ivector_online/final.mdl fst=tri3b/graph/HCLG.fst word-syms=tri3b/graph/words.txt \
 * feature-type=mfcc mfcc-config=nnet3_online_ivector_online/conf/mfcc.conf \
...
#include "./gstkaldinnet3onlinedecoder.h"
...
GST_DEBUG_CATEGORY_STATIC(gst_kaldinnet3onlinedecoder_debug);
#define GST_CAT_DEFAULT gst_kaldinnet3onlinedecoder_debug
...
static guint gst_kaldinnet3onlinedecoder_signals[LAST_SIGNAL];

#define gst_kaldinnet3onlinedecoder_parent_class parent_class
G_DEFINE_TYPE(Gstkaldinnet3onlinedecoder, gst_kaldinnet3onlinedecoder,
              GST_TYPE_ELEMENT);

static void gst_kaldinnet3onlinedecoder_load_phone_syms(Gstkaldinnet3onlinedecoder * filter,
                                                        const GValue * value);

static void gst_kaldinnet3onlinedecoder_load_word_syms(Gstkaldinnet3onlinedecoder * filter,
                                                       const GValue * value);

static void gst_kaldinnet3onlinedecoder_load_model(Gstkaldinnet3onlinedecoder * filter,
                                                   const GValue * value);

static void gst_kaldinnet3onlinedecoder_load_fst(Gstkaldinnet3onlinedecoder * filter,
                                                 const GValue * value);

static void gst_kaldinnet3onlinedecoder_load_lm_fst(Gstkaldinnet3onlinedecoder * filter,
                                                    const GValue * value);

static void gst_kaldinnet3onlinedecoder_load_big_lm(Gstkaldinnet3onlinedecoder * filter,
                                                    const GValue * value);

static void gst_kaldinnet3onlinedecoder_load_word_boundary_info(Gstkaldinnet3onlinedecoder * filter,
                                                                const GValue * value);


static void gst_kaldinnet3onlinedecoder_set_property(GObject * object,
                                                     guint prop_id,
                                                     const GValue * value,
                                                     GParamSpec * pspec);

static void gst_kaldinnet3onlinedecoder_get_property(GObject * object,
                                                     guint prop_id,
                                                     GValue * value,
                                                     GParamSpec * pspec);

static gboolean gst_kaldinnet3onlinedecoder_sink_event(GstPad * pad,
                                                       GstObject * parent,
                                                       GstEvent * event);

static GstFlowReturn gst_kaldinnet3onlinedecoder_chain(GstPad * pad,
                                                       GstObject * parent,
                                                       GstBuffer * buf);

static GstStateChangeReturn gst_kaldinnet3onlinedecoder_change_state(
    GstElement *element, GstStateChange transition);

static gboolean gst_kaldinnet3onlinedecoder_query(GstPad *pad, GstObject * parent, GstQuery * query);

static void gst_kaldinnet3onlinedecoder_finalize(GObject * object);

/* GObject vmethod implementations */

/* initialize the kaldinnet3onlinedecoder's class */
static void gst_kaldinnet3onlinedecoder_class_init(
    Gstkaldinnet3onlinedecoderClass * klass) {
  ...
  gobject_class->set_property = gst_kaldinnet3onlinedecoder_set_property;
  gobject_class->get_property = gst_kaldinnet3onlinedecoder_get_property;
  gobject_class->finalize = gst_kaldinnet3onlinedecoder_finalize;

  gstelement_class->change_state = gst_kaldinnet3onlinedecoder_change_state;
  ...
  gst_kaldinnet3onlinedecoder_signals[PARTIAL_RESULT_SIGNAL] = g_signal_new(
      "partial-result", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(Gstkaldinnet3onlinedecoderClass, partial_result),
      NULL,
      NULL, kaldi_marshal_VOID__STRING, G_TYPE_NONE, 1,
      G_TYPE_STRING);

  gst_kaldinnet3onlinedecoder_signals[FINAL_RESULT_SIGNAL] = g_signal_new(
      "final-result", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(Gstkaldinnet3onlinedecoderClass, final_result),
      NULL,
      NULL, kaldi_marshal_VOID__STRING, G_TYPE_NONE, 1,
      G_TYPE_STRING);

  gst_kaldinnet3onlinedecoder_signals[FULL_FINAL_RESULT_SIGNAL] = g_signal_new(
      "full-final-result", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(Gstkaldinnet3onlinedecoderClass, full_final_result),
      NULL,
      NULL, kaldi_marshal_VOID__STRING, G_TYPE_NONE, 1,
      G_TYPE_STRING);
 ...
 */
static void gst_kaldinnet3onlinedecoder_init(
    Gstkaldinnet3onlinedecoder * filter) {
  bool tmp_bool;
  int32 tmp_int;
  uint32 tmp_uint;
  float tmp_float;
  ...
  gst_pad_set_event_function(
      filter->sinkpad,
      GST_DEBUG_FUNCPTR(gst_kaldinnet3onlinedecoder_sink_event));
  gst_pad_set_chain_function(
      filter->sinkpad, GST_DEBUG_FUNCPTR(gst_kaldinnet3onlinedecoder_chain));
  gst_pad_set_query_function(
      filter->sinkpad, GST_DEBUG_FUNCPTR(gst_kaldinnet3onlinedecoder_query));
...
static void gst_kaldinnet3onlinedecoder_set_property(GObject * object,
  ...
  Gstkaldinnet3onlinedecoder *filter = GST_KALDINNET3ONLINEDECODER(object);
  ...
  switch (prop_id) {
    case PROP_SILENT:
      ...
      break;
    case PROP_MODEL:
      gst_kaldinnet3onlinedecoder_load_model(filter, value);
      break;
    case PROP_FST:
      gst_kaldinnet3onlinedecoder_load_fst(filter, value);
      break;
    case PROP_WORD_SYMS:
      gst_kaldinnet3onlinedecoder_load_word_syms(filter, value);
      break;
    case PROP_PHONE_SYMS:
      gst_kaldinnet3onlinedecoder_load_phone_syms(filter, value);
      break;
    ...
    case PROP_TRACEBACK_PERIOD_IN_SECS:
      ...
      break;
    case PROP_LM_FST:
      gst_kaldinnet3onlinedecoder_load_lm_fst(filter, value);
      break;
    case PROP_BIG_LM_CONST_ARPA:
      gst_kaldinnet3onlinedecoder_load_big_lm(filter, value);
      break;
    case PROP_WORD_BOUNDARY_FILE:
      gst_kaldinnet3onlinedecoder_load_word_boundary_info(filter, value);
      break;
...
static std::vector<PhoneAlignmentInfo> gst_kaldinnet3onlinedecoder_phone_alignment(
    Gstkaldinnet3onlinedecoder * filter, const std::vector<int32>& alignment) {
...
static std::vector<WordAlignmentInfo>  gst_kaldinnet3onlinedecoder_word_alignment(
    Gstkaldinnet3onlinedecoder * filter, const Lattice &lat) {
...
static void gst_kaldinnet3onlinedecoder_scale_lattice(
        Gstkaldinnet3onlinedecoder * filter, CompactLattice &clat) {
...
static std::string gst_kaldinnet3onlinedecoder_words_to_string(
    Gstkaldinnet3onlinedecoder *filter, const std::vector<int32> &words) {
...
static std::string gst_kaldinnet3onlinedecoder_words_in_hyp_to_string(
    Gstkaldinnet3onlinedecoder *filter, const std::vector<WordInHypothesis> &words) {
  std::vector<int32> word_ids;
  for (size_t i = 0; i < words.size(); i++) {
    word_ids.push_back(words[i].word_id);
  }
  return gst_kaldinnet3onlinedecoder_words_to_string(filter, word_ids);
}

static std::vector<NBestResult> gst_kaldinnet3onlinedecoder_nbest_results(
    Gstkaldinnet3onlinedecoder * filter, CompactLattice &clat) {

  std::vector<NBestResult> nbest_results;

  // FIXME: is it needed?
  //gst_kaldinnet3onlinedecoder_scale_lattice(filter, clat);
  Lattice lat;
    ...
    if (i == 0) {
      if (filter->do_phone_alignment) {
        nbest_result.phone_alignment =
            gst_kaldinnet3onlinedecoder_phone_alignment(filter, alignment);
      }
      if (filter->word_boundary_info) {
        nbest_result.word_alignment = gst_kaldinnet3onlinedecoder_word_alignment(filter, nbest_lats[i]);
      }
    }
...
static std::string gst_kaldinnet3onlinedecoder_full_final_result_to_json(
    Gstkaldinnet3onlinedecoder * filter,
  ...
  if (full_final_result.nbest_results.size() > 0) {
    ...
    for(std::vector<NBestResult>::const_iterator it = full_final_result.nbest_results.begin();
      ...
      json_object_set_new(nbest_result_json_object, "transcript",
                          json_string(gst_kaldinnet3onlinedecoder_words_in_hyp_to_string(filter, nbest_result.words).c_str()));
...
static void gst_kaldinnet3onlinedecoder_final_result(
    Gstkaldinnet3onlinedecoder * filter, CompactLattice &clat,
    guint *num_words) {
  if (clat.NumStates() == 0) {
  ...
  full_final_result.nbest_results = gst_kaldinnet3onlinedecoder_nbest_results(filter, clat);

  if (full_final_result.nbest_results.size() > 0) {
    std::string best_transcript = gst_kaldinnet3onlinedecoder_words_in_hyp_to_string(filter, full_final_result.nbest_results[0].words);
    ...
    *num_words = full_final_result.nbest_results[0].words.size();
    ...
    if (hyp_length > 0) {
      ...
      g_signal_emit(filter, gst_kaldinnet3onlinedecoder_signals[FINAL_RESULT_SIGNAL], 0, best_transcript.c_str());

      std::string full_final_result_as_json =
          gst_kaldinnet3onlinedecoder_full_final_result_to_json(filter, full_final_result);
      GST_DEBUG_OBJECT(filter, "Final JSON: %s", full_final_result_as_json.c_str());
      g_signal_emit(filter, gst_kaldinnet3onlinedecoder_signals[FULL_FINAL_RESULT_SIGNAL], 0, full_final_result_as_json.c_str());

    }
  }
}

static void gst_kaldinnet3onlinedecoder_partial_result(
    Gstkaldinnet3onlinedecoder * filter, const Lattice lat) {
  std::vector<int32> words;
  std::vector<int32> alignment;
  LatticeWeight weight;
  GetLinearSymbolSequence(lat, &alignment, &words, &weight);
  std::string transcript = gst_kaldinnet3onlinedecoder_words_to_string(filter, words);
  GST_DEBUG_OBJECT(filter, "Partial: %s", transcript.c_str());
  //if (transcript.length() > 0) {
    /* Emit a signal for applications. */
    g_signal_emit(filter,
                  gst_kaldinnet3onlinedecoder_signals[PARTIAL_RESULT_SIGNAL], 0,
                  transcript.c_str());
  //}
    
}

static bool gst_kaldinnet3onlinedecoder_rescore_big_lm(
    Gstkaldinnet3onlinedecoder * filter, CompactLattice &clat, CompactLattice &result_lat) {
...
static void gst_kaldinnet3onlinedecoder_unthreaded_decode_segment(Gstkaldinnet3onlinedecoder * filter,
                                                        bool &more_data,
                                                        int32 chunk_length,
  ...
  while (true) {
    ...
    if ((num_seconds_decoded - last_traceback > traceback_period_secs)
      ...
      gst_kaldinnet3onlinedecoder_partial_result(filter, lat);
  ...
  if (num_seconds_decoded > 0.1) {
    ...
    if ((filter->lm_fst != NULL) && (filter->big_lm_const_arpa != NULL)) {
      ...
      if (gst_kaldinnet3onlinedecoder_rescore_big_lm(filter, clat, rescored_lat)) {
        clat = rescored_lat;
      }
    }

    guint num_words = 0;
    gst_kaldinnet3onlinedecoder_final_result(filter, clat, &num_words);
...
static void gst_kaldinnet3onlinedecoder_loop(
    Gstkaldinnet3onlinedecoder * filter) {
  ...
  while (more_data) {
    gst_kaldinnet3onlinedecoder_unthreaded_decode_segment(filter, more_data, chunk_length, traceback_period_secs);
...
static gboolean
gst_kaldinnet3onlinedecoder_query (GstPad *pad, GstObject * parent, GstQuery * query) {
  gboolean ret;
  Gstkaldinnet3onlinedecoder *filter;

  filter = GST_KALDINNET3ONLINEDECODER(parent);
  ...
  switch (GST_EVENT_TYPE(event)) {
    case GST_EVENT_SEGMENT: {
      ...
      gst_pad_start_task(filter->srcpad,
                         (GstTaskFunction) gst_kaldinnet3onlinedecoder_loop,
                         filter, NULL);
...
static GstFlowReturn gst_kaldinnet3onlinedecoder_chain(GstPad * pad,
                                                       GstObject * parent,
                                                       GstBuffer * buf) {
  Gstkaldinnet3onlinedecoder *filter = GST_KALDINNET3ONLINEDECODER(parent);
...
static void
gst_kaldinnet3onlinedecoder_load_word_syms(Gstkaldinnet3onlinedecoder * filter,
                                           const GValue * value) {
    if (G_VALUE_HOLDS_STRING(value)) {
...
static void
gst_kaldinnet3onlinedecoder_load_phone_syms(Gstkaldinnet3onlinedecoder * filter,
                                            const GValue * value) {
  if (G_VALUE_HOLDS_STRING(value)) {
    gchar* str = g_value_dup_string(value);
...
static void
gst_kaldinnet3onlinedecoder_load_word_boundary_info(Gstkaldinnet3onlinedecoder * filter,
                                                    const GValue * value) {
  if (G_VALUE_HOLDS_STRING(value)) {
    gchar* str = g_value_dup_string(value);
...
static void
gst_kaldinnet3onlinedecoder_load_model(Gstkaldinnet3onlinedecoder * filter,
                                       const GValue * value) {
  if (G_VALUE_HOLDS_STRING(value)) {
    gchar* str = g_value_dup_string(value);
    ...
    if (strcmp(str, "") != 0) {
      ...
      if (!filter->nnet) {
        filter->nnet = new nnet3::AmNnetSimple();
      }
...
static void
gst_kaldinnet3onlinedecoder_load_fst(Gstkaldinnet3onlinedecoder * filter,
                                     const GValue * value) {
  if (G_VALUE_HOLDS_STRING(value)) {
    gchar* str = g_value_dup_string(value);
...
static void
gst_kaldinnet3onlinedecoder_load_lm_fst(Gstkaldinnet3onlinedecoder * filter,
                                        const GValue * value) {
  if (G_VALUE_HOLDS_STRING(value)) {
    gchar* str = g_value_dup_string(value);
...
static void
gst_kaldinnet3onlinedecoder_load_big_lm(Gstkaldinnet3onlinedecoder * filter,
                                        const GValue * value) {
  if (G_VALUE_HOLDS_STRING(value)) {
    gchar* str = g_value_dup_string(value);
...
static bool
gst_kaldinnet3onlinedecoder_allocate(
    Gstkaldinnet3onlinedecoder * filter) {
...
static bool gst_kaldinnet3onlinedecoder_deallocate(
    Gstkaldinnet3onlinedecoder * filter) {
...
static GstStateChangeReturn gst_kaldinnet3onlinedecoder_change_state(
  ...
  Gstkaldinnet3onlinedecoder *filter = GST_KALDINNET3ONLINEDECODER(element);

  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      if (!gst_kaldinnet3onlinedecoder_allocate(filter))
  ...
  switch (transition) {
    case GST_STATE_CHANGE_READY_TO_NULL:
      gst_kaldinnet3onlinedecoder_deallocate(filter);
      break;
    default:
      break;
  }
 ...
 */
static gboolean kaldinnet3onlinedecoder_init(
    GstPlugin * kaldinnet3onlinedecoder) {
  /* debug category for fltering log messages
   *
   * exchange the string 'Template kaldinnet3onlinedecoder' with your description
   */
  GST_DEBUG_CATEGORY_INIT(gst_kaldinnet3onlinedecoder_debug,
                          "kaldinnet3onlinedecoder", 0,
                          "Template kaldinnet3onlinedecoder");

  return gst_element_register(kaldinnet3onlinedecoder,
                              "kaldinnet3onlinedecoder", GST_RANK_NONE,
                              GST_TYPE_KALDINNET3ONLINEDECODER);
}
...
/* gstreamer looks for this structure to register kaldinnet3onlinedecoders
 *
 * exchange the string 'Template kaldinnet3onlinedecoder' with your kaldinnet3onlinedecoder description
 *
 * License is specified as "unknown" because gstreamer doesn't recognize "Apache" as
 * a license and blacklists the module :S
 */
GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, kaldinnet3onlinedecoder,
                  "kaldinnet3onlinedecoder",
                  kaldinnet3onlinedecoder_init, VERSION, "unknown", "GStreamer",
                  "http://gstreamer.net/")
